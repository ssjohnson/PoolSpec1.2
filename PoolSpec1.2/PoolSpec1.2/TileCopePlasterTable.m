//
//  TileCopePlasterTable.m
//  PoolSpec1.2
//
//  Created by Sean Johnson on 3/11/13.
//  Copyright (c) 2013 Sean Johnson. All rights reserved.
//

#import "TileCopePlasterTable.h"
#import "LTFCell.h"
#import "LSCell.h"
#import "NameImageViewCell.h"
#import "CustomTextField.h"

@interface TileCopePlasterTable ()

@end

@implementation TileCopePlasterTable
...
- (void)viewDidLoad
{
    ...
    itemArray = [[NSArray alloc] initWithObjects:
                      //Tiles
                      @"Replace / Reset Tiles", @"Tile Color", @"Tile Size", @"Tile Quantity",
                      //Cope
                      @"Stone Reset", @"Stone Replace", @"Re-Grout Stone",
                      //Plaster
                      @"Acid Wash", @"Whitecoat",nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if([indexPath section] == 0)
    {
        ...
        cell.cellTextField.text = [textArray objectAtIndex:0];
    ...
    else if([indexPath section] < 4)
    {
        if([indexPath row] > 0 && [indexPath section] == 1)
        {
            ...
            cell.cellTextField.text = [textArray objectAtIndex:[indexPath row]];
            [cell.cellTextField setDelegate:self];
...
- (void) textFieldDidEndEditing:(CustomTextField *)textField
{
    ...
    if([cell isMemberOfClass:[NameImageViewCell class]])
    {
        ...
        [textArray replaceObjectAtIndex:textField.index withObject:textField.text];
}
...
- (IBAction)sendEmail:(id)sender {
    if ([MFMailComposeViewController canSendMail])
    {
        ...
        [mailer setSubject:@"PoolSpec1.2 Test Email: Tile / Cope / Plaster Construction Request"];
        ...
        [emailBody appendFormat:@"Tile / Cope / Plaster Construction Request For: %@ \n\n", [textArray objectAtIndex:0]];
        ...
        for(int i = 0; i < [itemArray count]; i++)
        {
            if([[switchArray objectAtIndex:i]isEqualToNumber:[NSNumber numberWithInteger:1]])
            {
                ...
                if(i == 0)
                {
                    [emailBody appendFormat:@"%@ : %@\n", [itemArray objectAtIndex:i+1], [textArray objectAtIndex:i+1]];
                    [emailBody appendFormat:@"%@ : %@\n", [itemArray objectAtIndex:i+2], [textArray objectAtIndex:i+2]];
                    [emailBody appendFormat:@"%@ : %@\n", [itemArray objectAtIndex:i+3], [textArray objectAtIndex:i+3]];
                    i = 3;
                }
            }
        }
